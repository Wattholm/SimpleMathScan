...
class MathParser {
    ...
    static func parseArithmetic(fromText text: [String]) -> String? {
        ...
        guard let argument1 = validExpression.components(separatedBy: .decimalDigits.inverted).first else { return nil }
        guard let argument2 = validExpression.components(separatedBy: .decimalDigits.inverted).last else { return nil }
        let operandString = validExpression.trimmingCharacters(in: .decimalDigits)
        ...
        return compute(number1: number1, operand: operand, number2: number2)
    }
